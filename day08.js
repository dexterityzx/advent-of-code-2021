...
const patternsList = inputs.map((input) =>
  input.split("|")[0].trim().split(" ")
);

const numbersList = inputs.map((input) =>
  input.split("|")[1].trim().split(" ")
);

// part 1
// 1 4 7 8 can be identified by the digits length

const targetLength = new Set([2, 3, 4, 7]);

const count1 = numbersList
  .map((numbers) => numbers.filter((number) => targetLength.has(number.length)))
...
0. sort all string, including patterns and numbers strings
1. find 1's, 4's, 7's, 8's patterns by length
2. find 9's pattern by searching 
    1. length 6 
    2. include 1's, 4's, 7's patterns
3. find 6's pattern by searching 
    1. length 6 
    2. not include any of 1's, 4's, 7's patterns
4. find 0's pattern by searching 
    1. length 6 
    2. include 1's, 7's patterns
    3. not include 4's pattern
5. find 2's pattern by searching 
    1. length 5
    2. not include any of 1's patterns
    3. has more than 1 diff than 6's pattern
6. find 3's pattern by searching 
    1. length 5
    2. include 1's pattern
7. find 5's pattern by searching 
    1. length 5
    2. not include any of 1's patterns
    3. has ony 1 diff than 6's pattern
*/
const find0 = (patterns, pattern1, pattern4, pattern7) =>
  patterns
    .filter((pattern) => pattern.length == 6)
    .filter(
      (pattern) =>
        include(pattern, pattern1) &&
        !include(pattern, pattern4) &&
        include(pattern, pattern7)
    )
    .pop();
const find1 = (patterns) =>
  patterns.filter((pattern) => pattern.length == 2).pop();
const find2 = (patterns, pattern1, pattern6) =>
  patterns
    .filter((pattern) => pattern.length == 5)
    .filter((pattern) => !include(pattern, pattern1))
    .filter((pattern) => !oneDiff(pattern, pattern6))
    .pop();
const find3 = (patterns, pattern1) =>
  patterns
    .filter((pattern) => pattern.length == 5)
    .filter((pattern) => include(pattern, pattern1))
    .pop();
const find5 = (patterns, pattern1, pattern6) =>
  patterns
    .filter((pattern) => pattern.length == 5)
    .filter((pattern) => !include(pattern, pattern1))
    .filter((pattern) => oneDiff(pattern, pattern6))
    .pop();
const find4 = (patterns) =>
  patterns.filter((pattern) => pattern.length == 4).pop();
const find6 = (patterns, pattern1, pattern4, pattern7) =>
  patterns
    .filter((pattern) => pattern.length == 6)
    .filter(
      (pattern) =>
    ...
    .pop();
const find7 = (patterns) =>
  patterns.filter((pattern) => pattern.length == 3).pop();
const find8 = (patterns) =>
  patterns.filter((pattern) => pattern.length == 7).pop();
const find9 = (patterns, pattern1, pattern4, pattern7) =>
  patterns
    .filter((pattern) => pattern.length == 6)
    .filter(
      (pattern) =>
...
const include = (pattern1, pattern2) => {
  var p1Set = new Set(pattern1.split(""));
  for (let i = 0; i < pattern2.length; i++) {
    if (!p1Set.has(pattern2[i])) return false;
  }
  return true;
};

const oneDiff = (shortPattern, longPattern) => {
  if (longPattern.length - shortPattern.length > 1) return false;

  var longSet = new Set(longPattern.split(""));
  for (let i = 0; i < shortPattern.length; i++) {
    if (!longSet.has(shortPattern[i])) return false;
  }
  return true;
};

const findPatterns = (patterns) => {
  const pattern1 = find1(patterns);
  const pattern4 = find4(patterns);
  const pattern7 = find7(patterns);
  const pattern8 = find8(patterns);
  const pattern0 = find0(patterns, pattern1, pattern4, pattern7);
  const pattern6 = find6(patterns, pattern1, pattern4, pattern7);
  const pattern9 = find9(patterns, pattern1, pattern4, pattern7);
  const pattern2 = find2(patterns, pattern1, pattern6);
  const pattern3 = find3(patterns, pattern1);
  const pattern5 = find5(patterns, pattern1, pattern6);
  const map = new Map();
  [
    pattern0,
    pattern1,
    ...
    pattern8,
    pattern9,
  ].forEach((pattern, index) => map.set(pattern, index.toString()));
  return map;
};

const sortString = (item) => {
  var sortedItem = item.split("");
  sortedItem.sort();
  return sortedItem.join("");
};

const patternMaps = patternsList
...
const sum = numbersList
  ...
  .map((numbers, i) =>
    numbers.map((number) => patternMaps[i].get(number)).join("")
  )
